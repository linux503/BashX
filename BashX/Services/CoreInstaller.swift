...
enum CoreInstaller {
    ...
    private static let maxCompressedBytes = 40 * 1024 * 1024
    private static let maxDecompressedBytes = 64 * 1024 * 1024
    private static let binaryProbeTimeout: TimeInterval = 5
    ...
    private static func loadEmbeddedBinary() throws -> Data {
        ...
        stripQuarantine(at: src.path)
        if let attrs = try? FileManager.default.attributesOfItem(atPath: src.path),
           let size = attrs[.size] as? Int64, size > maxCompressedBytes {
            throw InstallError.extractFailed
        }
        let raw = try Data(contentsOf: src)
        guard raw.count <= maxCompressedBytes else { throw InstallError.extractFailed }
        if src.pathExtension == "gz" || src.lastPathComponent.hasSuffix(".gz") {
            return try gunzipData(raw)
        }
        guard raw.count <= maxDecompressedBytes else { throw InstallError.extractFailed }
        return raw
    }
    ...
    static func verifyInstalledBinary(at path: String) -> Bool {
        guard let attrs = try? FileManager.default.attributesOfItem(atPath: path),
              ...
              size > 0, size <= maxDecompressedBytes else { return false }
    ...
    /// Write `mihomo.sha256` when the binary matches the pinned release (sidecar may be missing after upgrades).
    static func recordHashIfPinnedMatch(at path: String) {
        guard let hex = sha256Hex(ofFile: path),
              let expected = expectedSHA256[hostArch],
              hex == expected else { return }
        try? hex.write(to: Paths.coreHashURL, atomically: true, encoding: .utf8)
        try? FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: Paths.coreHashURL.path)
    }
    ...
    /// Bypass system proxy — fresh install may not have proxy yet.
    ...
    private static func download(urlString: String) async throws -> Data {
        ...
        let (data, response) = try await directSession.data(from: url)
    ...
    private static func gunzip(_ url: URL) throws -> Data {
        ...
        var binary = Data()
        binary.reserveCapacity(32 * 1024 * 1024)
        let handle = pipe.fileHandleForReading
        while true {
            let chunk = handle.readData(ofLength: 262_144)
            if chunk.isEmpty { break }
            guard binary.count + chunk.count <= maxDecompressedBytes else {
    ...
    private static func binaryVersion(at path: String) -> String? {
        ...
        if sem.wait(timeout: .now() + binaryProbeTimeout) == .timedOut {
            if process.isRunning { process.terminate() }
            return nil
        }
        return result
