...
enum SubscriptionFetcher {
    ...
    /// Try several common client strings (same approach as Clash Verge / Stash).
    ...
    /// - Parameter viaProxyPort: when set (e.g. local mixed-port), fetch through HTTP proxy
    ...
    static func fetch(urlString: String, viaProxyPort: Int? = nil) async throws -> FetchResult {
        ...
        var bestData: Data?
        var bestUA = userAgents[0]
        var bestName: String?
        var lastError: Error?

        // 1) Direct (no inherited system proxy) — most airports return userinfo here.
        // Stop early once we have traffic info; otherwise try a few Clash-like UAs.
        for ua in userAgents.prefix(4) {
            do {
                let once = try await fetchOnce(url: url, userAgent: ua, proxyPort: nil)
                bestData = once.data
                bestUA = ua
                if bestName == nil { bestName = once.suggestedName }
                if let info = once.userInfo {
                    return FetchResult(
                        data: once.data,
                        userInfo: info,
                        usedUserAgent: ua,
                        suggestedName: once.suggestedName ?? bestName
                    )
                }
            } catch {
                lastError = error
        ...
        // 2) Via local mixed-port (Clash Verge `self_proxy`) if core is up.
        if let port = viaProxyPort, port > 0 {
            for ua in userAgents.prefix(2) {
                do {
                    let once = try await fetchOnce(url: url, userAgent: ua, proxyPort: port)
                    bestData = once.data
                    bestUA = ua
                    if bestName == nil { bestName = once.suggestedName }
                    if let info = once.userInfo {
        ...
        guard let data = bestData else {
            throw lastError ?? URLError(.badServerResponse)
        }
        return FetchResult(
            data: data,
            userInfo: SubscriptionUserInfo.parse(fromBody: data),
            usedUserAgent: bestUA,
