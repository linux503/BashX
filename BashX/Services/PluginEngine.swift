...
enum PluginEngine {
    ...
    private static let builtinFallback: [Plugin] = [
        ...
        Plugin(id: "cainiao-ads", name: "菜鸟去广告", summary: "拦截菜鸟裹裹全屏/开屏广告与广告流接口域名（物流主接口尽量保留）。", tag: "去广告", symbol: "shippingbox.fill", rules: [
            ...
            "DOMAIN-KEYWORD,adkeyword,REJECT",
        ], scriptHeavy: false),
        Plugin(id: "amap-ads", name: "高德地图去广告", summary: "拦截高德开屏素材与广告 CDN（不拦 m5 主接口，避免影响导航）。", tag: "去广告", symbol: "map.fill", rules: [
            ...
            "DOMAIN,ai.amap.com,REJECT",
            "DOMAIN,oss.amap.com,REJECT",
            "DOMAIN,sns.amap.com,REJECT",
