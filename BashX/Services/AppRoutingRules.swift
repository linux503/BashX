...
enum AppRoutingRules {
    ...
    struct CommonAppPreset: Identifiable, Hashable {
        ...
        func asRule(enabled: Bool = true) -> AppRoutingRule {
            AppRoutingRule(
                enabled: enabled,
                label: label(lang: .current),
    ...
    static let commonPresets: [CommonAppPreset] = [
        ...
        .init(category: .dev, labelZh: "Cursor", labelEn: "Cursor",
              processName: "Cursor", bundleId: "com.todesktop.230313mzl4w4u92", proxyTarget: "PROXY"),
    ...
    static func clashRules(from rules: [AppRoutingRule]) -> [String] {
        ...
        for rule in rules where rule.enabled {
            ...
            let bundle = rule.bundleId.trimmingCharacters(in: .whitespacesAndNewlines)
            if !bundle.isEmpty {
                appendUnique(&out, &seen, "PROCESS-NAME,\(escape(bundle)),\(target)")
            }
            let label = rule.label.trimmingCharacters(in: .whitespacesAndNewlines)
            if !label.isEmpty, !label.contains(",") {
                appendUnique(&out, &seen, "PROCESS-PATH,*\(label).app/Contents/MacOS/*,\(target)")
            }
        }
        return out
    }
