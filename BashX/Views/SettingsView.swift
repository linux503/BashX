    ...
    private var generalTab: some View {
        Form {
            Section("代理模式") {
                ...
                Text("增强模式，可接管更多流量；首次开启会请求管理员权限。提权前会校验内核完整性。")
                    .font(.caption)
                    .foregroundStyle(BashXTheme.secondaryLabel(for: appearance))
    ...
    private var appearanceTab: some View {
        Form {
            Section("外观") {
                ...
                TextField("菜单栏快速切节点数量", value: $state.settings.menuNodeLimit, format: .number)

                Toggle("菜单栏显示网速", isOn: Binding(
                    get: { state.settings.showMenuBarTraffic },
                    set: { state.setShowMenuBarTraffic($0) }
                ))
                Text("在菜单栏 Logo 旁显示 ↓/↑ 实时速率。也可在菜单「设置 → 显示网速」切换。")
                    .font(.caption)
                    .foregroundStyle(BashXTheme.secondaryLabel(for: appearance))
