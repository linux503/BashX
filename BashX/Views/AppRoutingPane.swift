...
struct AppRoutingPane: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    if !state.settings.appRoutingRules.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            LazyVStack(spacing: 8) {
                                ForEach(state.settings.appRoutingRules) { rule in
                                    AppRoutingRow(
                                        rule: rule,
                                        lang: lang,
        ...
        .sheet(isPresented: $showPicker) {
            RunningAppPicker(
                ...
                lang: lang,
                appearance: appearance,
                existing: Set(state.settings.appRoutingRules.map(\.bundleId).filter { !$0.isEmpty })
            ) { app, target in
    ...
    private func presetChip(_ preset: AppRoutingRules.CommonAppPreset) -> some View {
        let installed = AppRoutingRules.presetInstalled(preset, in: state.settings.appRoutingRules)
            ...
            List(filtered) { app in
                Button {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Spacer()
                        if existing.contains(app.bundleId), !app.bundleId.isEmpty {
