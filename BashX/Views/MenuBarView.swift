        ...
        Menu(snap.subscriptionMenuTitle) {
            if snap.subscriptions.isEmpty {
                ...
                ForEach(snap.subscriptions) { sub in
                    Button {
                        Task { await state.switchToSubscription(sub.id) }
                    } label: {
                        menuCheckRow(
                            snap.subscriptionRowTitle(sub),
                            on: sub.enabled && snap.onlyThisEnabled(sub)
                        )
                    }
                }
                Divider()
                Text("单独更新")
                    .foregroundStyle(.secondary)
                ForEach(snap.subscriptions) { sub in
                    Button("更新 \(snap.shortName(sub.name, 12))") {
                        Task { await state.updateSubscription(sub.id) }
                    }
                ...
                ForEach(snap.subscriptions) { sub in
                    Toggle(isOn: Binding(
                        ...
                        Text(snap.shortName(sub.name, 14))
                    }
                }
                Divider()
            }
...
private struct MenuBarSnapshot: Equatable {
    ...
    static func capture(from state: AppState) -> MenuBarSnapshot {
        ...
        let subTitle: String = {
            ...
            if let current = subs.first(where: { $0.enabled && subs.filter(\.enabled).count == 1 }) {
                return "订阅 · \(shortName(current.name, 8))"
            }
            return "订阅 \(enabledCount)/\(subs.count)"
        }()
        let hint: String = {
    ...
    func subscriptionRowTitle(_ sub: Subscription) -> String {
        var parts: [String] = [shortName(sub.name, 14)]
        if !sub.enabled {
            parts.append("关")
        } else if let info = sub.userInfo {
            parts.append("剩\(info.remainingText)")
        }
        return parts.joined(separator: " · ")
    }

    func onlyThisEnabled(_ sub: Subscription) -> Bool {
        sub.enabled && subscriptions.filter(\.enabled).count == 1
    }
