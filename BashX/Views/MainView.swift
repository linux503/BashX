...
struct MainView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showAddSubscription) {
            BashXThemed(appearance: state.settings.appearance) {
                AddSubscriptionSheet(isPresented: $showAddSubscription)
                    .environmentObject(state)
            }
        }
        .onChange(of: state.panelIntent) { _, intent in
            switch intent {
            case .none:
                break
            case .subscriptions:
        ...
        HStack(spacing: 12) {
            HStack(spacing: 10) {
                Button {
                    ...
                    LogoIconView(style: state.settings.logoStyle, size: 28, colored: true, panel: true)
    ...
    private var selectedNodeCard: some View {
        ...
        .onChange(of: state.coreRunning) { _, running in
            if running {
    ...
    private var nodesPaneContent: some View {
        VStack(spacing: 0) {
            ...
            if state.filteredNodes.isEmpty {
                nodesEmptyState
            } else if state.settings.nodeDisplayMode == .card {
                nodesCardPane
            } else {
                nodesListPane
            }
    ...
    private var nodesListPane: some View {
        VStack(spacing: 0) {
            ...
            List(selection: Binding(
                get: { state.settings.selectedNodeName },
                set: { name in
                    if let name {
                        selectNodeFromPanel(name)
                    }
                }
            )) {
                ForEach(state.categoryGroups) { group in
                    Section {
                        if !state.collapsedCategories.contains(group.key) {
    ...
    private var nodesCardPane: some View {
        ScrollView {
    ...
    private func categoryHeader(_ group: NodeCategory.Group) -> some View {
        let collapsed = state.collapsedCategories.contains(group.key)
        return Button {
            state.toggleCategoryCollapsed(group.key)
        } label: {
            HStack(spacing: 8) {
                ...
                Spacer(minLength: 4)
                if let best = group.nodes.compactMap(\.delayMs).filter({ $0 > 0 }).min() {
                    Text("最快 \(best)ms")
    ...
    private func categoryHeaderCard(_ group: NodeCategory.Group) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 10) {
                Image(systemName: collapsed ? "chevron.right.circle.fill" : "chevron.down.circle.fill")
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(BashXTheme.accent(for: appearance))
                Text(group.flag)
                    .font(.system(size: 16))
    ...
    private func nodeCard(_ node: ProxyNode, group: NodeCategory.Group) -> some View {
        ...
        return Button {
            selectNodeFromPanel(node.name)
        } label: {
            VStack(alignment: .leading, spacing: 5) {
                HStack(alignment: .center, spacing: 6) {
                    ZStack {
                        Circle()
                            .fill(BashXTheme.accentSoft(for: appearance).opacity(highlighted ? 1 : 0.7))
                ...
                HStack(spacing: 5) {
                    Text(shortType(node.type))
                        ...
                        .background(
                            Capsule(style: .continuous)
                                .fill(highlighted
                                      ? BashXTheme.accent(for: appearance).opacity(appearance == .dark ? 0.22 : 0.12)
                ...
                RoundedRectangle(cornerRadius: 9, style: .continuous)
                    .fill(
                        LinearGradient(
                            colors: highlighted
                                ...
                                : [
                                    BashXTheme.card(for: appearance),
                                    BashXTheme.field(for: appearance).opacity(appearance == .dark ? 0.55 : 0.35)
                                ],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 9, style: .continuous)
                            .strokeBorder(
                                highlighted
                                    ? BashXTheme.accent(for: appearance).opacity(appearance == .dark ? 0.65 : 0.42)
                                    : BashXTheme.separator(for: appearance),
                                lineWidth: highlighted ? 1 : (appearance == .dark ? 0.8 : 0.5)
                            )
                    )
                    .shadow(
                        color: highlighted
                            ? BashXTheme.accent(for: appearance).opacity(appearance == .dark ? 0.22 : 0.12)
                            : Color.black.opacity(appearance == .dark ? 0.45 : 0.03),
                        radius: highlighted ? 5 : 2,
                        y: highlighted ? 2 : 1
                    )
            }
        }
    ...
    private var rulesPane: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            TextEditor(text: rulesTextBinding)
            ...
            if !rulesIssues.isEmpty {
                VStack(alignment: .leading, spacing: 2) {
                    ForEach(Array(rulesIssues.prefix(4).enumerated()), id: \.offset) { _, issue in
                        Text("· \(issue)")
                            .font(.caption2)
                            .foregroundStyle(BashXTheme.warn(for: appearance))
                    }
                    if rulesIssues.count > 4 {
                        Text("…另有 \(rulesIssues.count - 4) 条提示")
                            .font(.caption2)
                            .foregroundStyle(BashXTheme.tertiaryLabel(for: appearance))
                    }
                }
            ...
            HStack(spacing: 8) {
                Button("应用智能规则 v\(ChinaSmartRules.version)") {
                    Task { await state.applyChinaSmartRules() }
                }
                .controlSize(.small)
                ...
                Button("保存并生效") {
                    Task { await state.saveRulesFromEditor() }
                }
                .controlSize(.small)
                .keyboardShortcut(.defaultAction)
                .disabled(!rulesDirty && rulesIssues.isEmpty)
            }
        }
        .padding(16)
        .onAppear { state.ensureRulesTextLoaded() }
    }

    private var rulesDirty: Bool {
        ClashRuleSyntax.parseLines(state.rulesText) != state.settings.rules
    }

    private var rulesIssues: [String] {
        ClashRuleSyntax.validate(state.rulesText)
    }

    private var rulesStatusLine: String {
        let edited = ruleLineCount
        let runtime = state.effectiveRuntimeRules().count
        var parts = ["编辑 \(edited) 条", "生效 \(runtime) 条"]
    ...
    private var ruleLineCount: Int {
        ClashRuleSyntax.parseLines(state.rulesText).count
    }
    ...
    private var rulesTextBinding: Binding<String> {
        Binding(get: { state.rulesText }, set: { state.rulesText = $0 })
    }
}
