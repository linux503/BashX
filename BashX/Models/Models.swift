...
struct AppSettings: Codable {
    ...
    /// Sticky node for CURSOR / OPENAI / ANTHROPIC select groups (not moved by auto-fastest).
    var stableAINodeName: String? = nil
    /// Circuit-broken node keys (`delayCacheKey`) excluded from url-test pools.
    var isolatedNodeKeys: [String] = []
    ...
    /// Migrate removed teal/cyan logo styles back to brand yellow mark.
    private static func decodeLogoStyle(from c: KeyedDecodingContainer<CodingKeys>) -> LogoStyle {
        if let style = try? c.decode(LogoStyle.self, forKey: .logoStyle) {
            return style
        }
        guard let raw = try? c.decode(String.self, forKey: .logoStyle) else {
            return .default
        }
        switch raw {
        case "wave", "globe":
            return .markX
        default:
            return LogoStyle(rawValue: raw) ?? .default
        }
    }
}
