...
echo "[0/6] Fetch embedded mihomo cores…"
...
echo "[4/6] First-open helper + 说明…"
# AppleScript .app — clearer than .command for non-technical users.
HELPER_SCRIPT="$STAGE/_unlock.applescript"
...
on run
  ...
  if (do shell script "[ -d " & quoted form of dmgApp & " ] && echo 1 || echo 0") is "1" then
    ...
    display dialog "未找到 BashX.app。请先打开 DMG，保持本助手与 BashX 在同一窗口后再试。" buttons {"好"} default button 1 with icon stop
    return
  end if

  -- Clear Gatekeeper quarantine (why other Macs show「已损坏」)
  ...
  display dialog "✅ 已安装到「应用程序」并解除隔离。

若仍弹「无法打开 / 已损坏」：
系统设置 → 隐私与安全性 → 仍要打开

以后可从启动台或应用程序正常打开 BashX。" buttons {"好"} default button 1 with icon note
end run
APPLESCRIPT

osacompile -o "$STAGE/一键解锁并打开.app" "$HELPER_SCRIPT" >/dev/null
rm -f "$HELPER_SCRIPT"
# Ad-hoc sign helper so Gatekeeper is slightly less angry
...
其他电脑打不开 / 提示「已损坏」？
══════════════════════════════════════
这是 macOS 安全机制（未苹果公证时必现），不是文件坏了。

【推荐】右键点击「一键解锁并打开」→ 打开
  （首次可能再点「仍要打开」）
  脚本会：复制到应用程序 + 清除隔离标记 + 启动

【或】终端执行：
  xattr -cr /Applications/BashX.app
  open /Applications/BashX.app

【或】系统设置 → 隐私与安全性 → 仍要打开

──────────────────────────────────────
永久免此步骤（给你发软件的人）：
在 Apple 开发者账号创建「Developer ID Application」证书，
并用 NOTARY_PROFILE 公证后再分发。
详见项目 docs/distribute.md
EOF

# Keep legacy .command as backup
cat > "$STAGE/首次打开（备用）.command" <<'EOF'
#!/bin/bash
cd "$(dirname "$0")"
APP_SRC="BashX.app"
APP_DST="/Applications/BashX.app"
if [[ -d "$APP_SRC" ]]; then
  rm -rf "$APP_DST"
...
open "$APP_DST"
osascript -e 'display dialog "已解除隔离并打开 BashX。若仍失败：系统设置 → 隐私与安全性 → 仍要打开。" buttons {"好"} default button 1' >/dev/null 2>&1 || true
EOF
chmod +x "$STAGE/首次打开（备用）.command"
