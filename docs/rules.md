...
版本：**v10** · 更新：2026-08-27

## 策略说明

参考 **ACL4SSR**、**Clash Verge**、**ClashX Pro** 的分层思路，按优先级匹配：

| 优先级 | 策略 | 说明 |
|--------|------|------|
| 1 | LAN / 本地 | `.local` → REJECT（避免 mDNS 日志刷屏），私网 IP 直连 |
| 2 | 系统检测 | Captive Portal、NTP、Apple 推送直连 |
| 3 | 即时通讯 | Telegram / Discord / Slack / Signal 进程 + GeoIP 兜底走代理 |
| 4 | 国内域名 | 常用站点 + `GEOSITE,cn` 直连 |
| 5 | 广告 / 成人 | `category-ads-all`、`category-porn` → REJECT |
| 6 | 国外服务 | Google / YouTube / GitHub / AI / 流媒体 / 开发工具走代理 |
| 7 | GFW | `gfw` / `greatfire` / `geolocation-!cn` 走代理 |
| 8 | 兜底 | 中国 IP 直连，其余走代理 |

App 内开启 **视频广告过滤** 时，会额外合并 `VideoAdBlock` 域名 REJECT（B站 / YouTube / 抖音等）。
