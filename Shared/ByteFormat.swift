...
enum ByteFormat {
    ...
    static func menuBarFixed(_ bytesPerSec: Int64) -> String {
        ...
        if v < 1024 * 1024 {
            ...
            if k < 10 {
                raw = String(format: "%.1fK", k)
            } else {
                raw = String(format: "%.0fK", k)
            }
        } else if v < 1024 * 1024 * 1024 {
            let m = min(v / (1024 * 1024), 999)
            if m < 10 {
                raw = String(format: "%.1fM", m)
            } else {
                raw = String(format: "%.0fM", m)
            }
        } else {
            let g = min(v / (1024 * 1024 * 1024), 99)
            raw = g < 10 ? String(format: "%.1fG", g) : String(format: "%.0fG", g)
        }
        if raw.count >= 4 { return String(raw.prefix(4)) }
        let pad = String(repeating: "\u{2007}", count: 4 - raw.count)
        return pad + raw
    }
