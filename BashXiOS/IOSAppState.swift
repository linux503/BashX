...
final class IOSAppState: ObservableObject {
    ...
    init() {
        ...
        Task { [weak self] in
            ...
            if !GeoDataBootstrap.isReady() {
                self.isPreparingGeodata = true
                self.statusText = "正在准备地理数据库…"
                defer { self.isPreparingGeodata = false }
                do {
                    try await GeoDataBootstrap.ensurePresent { msg in
                        Task { @MainActor in self.statusText = msg }
                    }
                } catch {
                    self.statusText = "地理库下载失败：\(error.localizedDescription)"
                }
            }
        }
    ...
    func refreshOutboundIP() async {
        ...
        if let ip = await OutboundIPProbe.fetch(viaProxyPort: nil) {
            outboundIP = ip
        } else if let ip = await OutboundIPProbe.fetch(viaProxyPort: AppConstants.mixedPort) {
            outboundIP = ip
        } else {
            outboundIP = "查询失败"
        }
    ...
    func toggleVPN() async {
        ...
        guard FileManager.default.fileExists(atPath: Paths.mihomoConfigURL.path) else {
            statusText = "请先更新订阅并选择节点"
            return
        }
        if !vpn.isConnected {
            if !GeoDataBootstrap.isReady() {
                isPreparingGeodata = true
                statusText = "正在准备地理数据库…"
                defer { isPreparingGeodata = false }
                do {
                    try await GeoDataBootstrap.ensurePresent { msg in
                        Task { @MainActor in self.statusText = msg }
                    }
                } catch {
                    statusText = "地理库准备失败：\(error.localizedDescription)"
                    return
                }
            }
        }
