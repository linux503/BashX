...
final class IOSAppState: ObservableObject {
    ...
    /// Single-subscription update in flight (nil when idle). Avoids labeling every row as updating.
    @Published var updatingSubscriptionID: UUID? = nil
    ...
    func updateSubscription(id: UUID, showBusy: Bool = true) async -> Bool {
        ...
        defer {
            if showBusy, updatingSubscriptionID == id {
                updatingSubscriptionID = nil
            }
        }
    ...
    func setIosOnDemandEnabled(_ enabled: Bool) {
        guard settings.iosOnDemandEnabled != enabled else { return }
        settings.iosOnDemandEnabled = enabled
        UserDefaults(suiteName: AppConstants.appGroupIdentifier)?
            .set(enabled, forKey: "iosOnDemandEnabled")
        persistNow()
        Task { await vpn.syncOnDemandPreference(enabled: enabled) }
        statusText = enabled ? L10n.t("ios.ondemand.on") : L10n.t("ios.ondemand.off")
        UISelectionFeedbackGenerator().selectionChanged()
    }
    ...
    func toggleVPN() async {
        ...
        // Global mode ignores DOMAIN/IP rules — WeChat image CDN would go overseas and fail.
        if !vpn.isConnected, settings.proxyMode == .global {
            settings.proxyMode = .rule
            persist()
